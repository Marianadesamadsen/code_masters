...
plot_dir = r"./results/test/plots"
anim_dir = r"./results/test/animations"
...
uhat_1step, utrue_1step, err_1step, abs_time = helper.one_step_all_batches(
    pred_all, target_all, time_all
)

# Full rollout metrics
# Average over nodes and features, keep rollout and time
errors_rollout = helper.compute_errors(pred_all, target_all, axis=(3, 4))
...
uhat_1wave, utrue_1wave, err_1wave, elapsed_time = helper.get_single_rollout(
    pred_all, target_all, time_all, wave=0, rollout_idx=0, feature=0
)

# 1-step scalar metrics over 1 wave and nodes
errors_1step = helper.compute_errors(uhat_1wave, utrue_1wave, axis=1)
err_1step = errors_1step["err"]         # shape: (n_time_steps, node)
...
ds_pred = helper.setup_simple_xarray(uhat_1wave, elapsed_time, P, tri, R=R)
ds_true = helper.setup_simple_xarray(utrue_1wave, elapsed_time, P, tri, R=R)
ds_err = helper.setup_simple_xarray(err_1wave, elapsed_time, P, tri, R=R)
...
fig2 = plot_funcs.plot_error_metrics(
    rmse_1step, mae_1step, max_err_1step, elapsed_time)
...
figs4 = plot_funcs.plot_rmse_heatmap(pred_all, target_all)
for i, fig in enumerate(figs4):
    fig.savefig(os.path.join(plot_dir, f"plot_rmse_heatmap_overall_{i}.png"), dpi=200, bbox_inches="tight")

fig5 = plot_funcs.plot_rollout_error_growth(pred_all, target_all)
for i, fig in enumerate(fig5):
    fig.savefig(os.path.join(plot_dir, f"plot_rollout_error_growth_overall_{i}.png"), dpi=200, bbox_inches="tight")
