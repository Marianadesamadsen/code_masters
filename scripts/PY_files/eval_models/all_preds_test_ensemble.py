...
sys.path.insert(0, "./")
import scripts.PY_files.eval_models.helper_functions_ensemble as helper
...
# paths
ds_geo = xr.open_dataset(r"./data/nc_files/wave_ensemble_10_coarse.nc")
...
elapsed_time_unique, rmse_mean, rmse_std = helper.aggregate_metric_by_time(
    rmse_1step, abs_time
)
_, mae_mean, mae_std = helper.aggregate_metric_by_time(mae_1step, abs_time)
_, max_mean, max_std = helper.aggregate_metric_by_time(max_err_1step, abs_time)
_, rel_rmse_mean, rel_rmse_std = helper.aggregate_metric_by_time(rel_rmse_1step, abs_time)
...
uhat_roll, utrue_roll, err_roll, elapsed_roll = helper.get_single_rollout(
    pred_all, target_all, time_all, rollout_idx=0
)

ds_pred = helper.setup_simple_xarray(uhat_roll, elapsed_roll, P, tri, R=R)
ds_true = helper.setup_simple_xarray(utrue_roll, elapsed_roll, P, tri, R=R)
ds_err = helper.setup_simple_xarray(err_roll, elapsed_roll, P, tri, R=R)
...
fig2 = plot_funcs.plot_error_metrics(
    rmse_mean, mae_mean, max_mean, rel_rmse_mean, elapsed_time_unique
)
