...
ENERGY_FILENAME = "test_energy_rel_error_sem_reference_per_sample.csv"
...
RUN_PAIRS = {
    10: {"label": r"$10\Delta t$", "ar1_dir": "test_10dt_2", "ar2_dir": "test_10dt_AR_2"},
    20: {"label": r"$20\Delta t$", "ar1_dir": "test_20dt_2", "ar2_dir": "test_20dt_AR_2"},
    40: {"label": r"$40\Delta t$", "ar1_dir": "test_40dt_2", "ar2_dir": "test_40dt_AR_2"},
}
...
def load_metric(result_dir, filename):
    path = BASE_DIR / result_dir / filename
    if not path.exists():
...
def compute_percentage_improvement(ar1_df, ar2_df):
    _, ar1_rollouts = get_rollout_cols(ar1_df)
    _, ar2_rollouts = get_rollout_cols(ar2_df)
    ...
    for rollout in common_rollouts:
        col = f"rollout_{rollout}"

        ar1_mean = ar1_df[col].mean()
        ar2_mean = ar2_df[col].mean()
...
def plot_ar2_percentage_improvement():
    ...
    for dt_scale, cfg in RUN_PAIRS.items():
        ...
        rollouts, rmse_improvement = compute_percentage_improvement(ar1_rmse, ar2_rmse)
        x_physical = rollouts * dt_scale * DT_BASE
        ...
        # SEM-reference relative energy error improvement
        ar1_energy = load_metric(cfg["ar1_dir"], ENERGY_FILENAME)
        ar2_energy = load_metric(cfg["ar2_dir"], ENERGY_FILENAME)

        rollouts, energy_improvement = compute_percentage_improvement(ar1_energy, ar2_energy)
        x_physical = rollouts * dt_scale * DT_BASE
    ...
    out_path = RESULTS_DIR / "ar2_percentage_improvement_sem_reference.png"
