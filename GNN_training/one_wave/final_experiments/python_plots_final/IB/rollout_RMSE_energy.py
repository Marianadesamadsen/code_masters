...
RESULTS_DIR = Path("GNN_training/one_wave/different_mesh_size/final_results_plots/final/IB")
...
RUN_DIRS = {
    "1dt": {"label": r"", "result_dir": "test_1dt", "dt_scale": 1},
}

def compute_true_rms_curve_from_nc(u, rollouts, dt_scale):
    """
    Computes the RMS magnitude of the true solution for each rollout horizon.
    """
    ...
    for rollout in rollouts:
        ...
        for member in member_indices:
            ...
            rms_per_start = np.sqrt(np.mean(uh**2, axis=1))
            rms_values.append(rms_per_start)

        rms_values = np.concatenate(rms_values)
        rms_curve.append(np.mean(rms_values))

    return np.array(rms_curve)
...
def compute_persistence_rmse_curve_from_nc(u, rollouts, dt_scale):
    ...
    Persistence prediction:
        u_pers(t0 + horizon) = u(t0)

    horizon = rollout_number * dt_scale

    This averages over the same test members and start times sampled with
    TEST_TIME_STRIDE.
    """
...
def plot_rollout_rmse_energy():
    ...
    for dt_key, cfg in RUN_DIRS.items():
        ...
        true_rms = compute_true_rms_curve_from_nc(
            u=u,
            rollouts=rmse_rollouts,
            dt_scale=dt_scale,
        )

        #y_rmse = y_rmse / true_rms      # Relative RMSE
        # or
        #y_rmse = 100 * y_rmse / true_rms   # Percentage
        if dt_key == "100dt":
            x_rmse = x_rmse[:-1]
            y_rmse = y_rmse[:-1]

        print(f"RMSE: model {dt_key} model one step:",y_rmse[0])
        print(f"RMSE: model {dt_key} model tot:",y_rmse)
        axes[0].loglog(
            x_rmse,
            y_rmse,
            marker="o",
        ...
        if dt_scale == -1:
            ...
            axes[0].loglog(
                ...
                linestyle=":",
                linewidth=2.0,
                label=rf"Persistence",
            )
            print("persistence RMSE:",y_persistence_rmse)
        ...
        print(f"REE: model {dt_key} model one step:",y_energy[0])
        print(f"REE: model {dt_key} model tot:",y_energy)
        ...
        if dt_scale == -1:
            ...
            axes[1].loglog(
                x_energy[:len(y_persistence_energy)],
            ...
            print("persistence energy:",y_persistence_energy)
