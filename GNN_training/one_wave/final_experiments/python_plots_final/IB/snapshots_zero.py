...
RUNS = {
    ...
    f"{dt_type}dt 2": {
        "label": rf"$\hat{{u}}_{{t+2\cdot {dt_type}\Delta t}}$",
        "label2": rf"$u_{{t+2\cdot {dt_type}\Delta t}}$",
        "zarr_path": BASE_DIR / f"test_{dt_type}dt_zero.zarr",
        "rollout_idx": 1,
    },
    f"{dt_type}dt 3": {
        "label": rf"$\hat{{u}}_{{t+3\cdot{dt_type}\Delta t}}$",
        "label2": rf"$u_{{t+3\cdot{dt_type}\Delta t}}$",
        "zarr_path": BASE_DIR / f"test_{dt_type}dt_zero.zarr",
        "rollout_idx": 2,
    },
        f"{dt_type}dt 4": {
        "label": rf"$\hat{{u}}_{{t+4\cdot {dt_type}\Delta t}}$",
        "label2": rf"$u_{{t+4\cdot {dt_type}\Delta t}}$",
        "zarr_path": BASE_DIR / f"test_{dt_type}dt_zero.zarr",
        "rollout_idx": 3,
    },}
...
feature_idx = 0
save_name = f"latlon_triangular_{dt_type}dt_prediction_sampleidx{sample_idx}_zero.png"
...
def main():
    ...
    for row, run_key in enumerate(RUNS):
        ...
        axes[row,0].tick_params(axis="x", labelsize=18)
        axes[row,0].tick_params(axis="y", labelsize=18)
        axes[row, 1].tick_params(axis="x", labelsize=18)
        axes[row, 1].tick_params(axis="y", labelsize=18)
        axes[row, 2].tick_params(axis="x", labelsize=18)
        axes[row, 2].tick_params(axis="y", labelsize=18)
