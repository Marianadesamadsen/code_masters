...
RUN_DIRS = {
    "10dt": {
        "label": r"$10\Delta t$",
        "result_dir": "test_10dt_2",
        "dt_scale": 10,
        "line": "-",
        "color": "red",
    },
    "10dt sub 1": {
        "label": r"$10\Delta t$ sub 1",
        "result_dir": "test_10dt_sub1",
        "dt_scale": 10,
        "line": "--",
        "color": "red",
    },
    ...
    "80dt": {
        "label": r"$80\Delta t$",
        "result_dir": "test_80dt_2",
        "dt_scale": 80,
        "line": "-",
        "color": "blue",
    },
    "80dt sub 1": {
        "label": r"$80\Delta t$ sub 1",
        "result_dir": "test_80dt_sub1_cont",
        "dt_scale": 80,
        "line": "--",
        "color": "blue",
    },
...
def compute_relative_energy_error_from_sem_nc(
    ...
    return rel_error_df, true_energy_df, pred_energy_df, energy_rollouts
...
def recompute_and_save_sem_energy_errors():
    ...
    for dt_key, cfg in RUN_DIRS.items():
        ...
        rel_energy_df, true_energy_df, pred_energy_df_clean, _ = (
        ...
        pred_energy_df_clean.to_csv(
            model_save_dir / "test_energy_pred_clean_per_sample.csv",
            index=False,
        )

        sem_rel_errors[dt_key] = rel_energy_df

    return sem_rel_errors
...
def plot_rollout_rmse_energy():
    ...
    axes[1].set_ylabel("SEM relative energy error", fontsize=20)
    ...
    out_path = RESULTS_DIR / "rmse_energy_sem_reference_10dt80dt.png"
...
def compute_mean_percentage_improvements():
    data_rmse = load_metric("test_rmse_per_sample.csv")
    data_energy_rel = recompute_and_save_sem_energy_errors()
    ...
    print("\nMean percentage improvement from baseline to sub1")
    print("Using SEM-reference relative energy error")
    print("-" * 60)

    for base_key, sub1_key in pairs:
        # RMSE
        base_rmse_df = data_rmse[base_key]
        sub1_rmse_df = data_rmse[sub1_key]

        rmse_cols, _ = get_rollout_cols(base_rmse_df)

        base_rmse = base_rmse_df[rmse_cols].mean(axis=0).values
        sub1_rmse = sub1_rmse_df[rmse_cols].mean(axis=0).values

        rmse_improvement = 100.0 * (base_rmse - sub1_rmse) / base_rmse
        mean_rmse_improvement = np.nanmean(rmse_improvement)

        # SEM energy error
        base_energy_df = data_energy_rel[base_key]
        sub1_energy_df = data_energy_rel[sub1_key]

        energy_cols, _ = get_rollout_cols(base_energy_df)

        base_energy = base_energy_df[energy_cols].mean(axis=0).values
        sub1_energy = sub1_energy_df[energy_cols].mean(axis=0).values

        energy_improvement = 100.0 * (base_energy - sub1_energy) / base_energy
        mean_energy_improvement = np.nanmean(energy_improvement)

        print(f"{base_key} -> {sub1_key}")
        print(f"  Mean RMSE improvement:             {mean_rmse_improvement:.2f}%")
        print(f"  Mean SEM energy error improvement: {mean_energy_improvement:.2f}%")
        print()


if __name__ == "__main__":
    plot_rollout_rmse_energy()
    compute_mean_percentage_improvements()