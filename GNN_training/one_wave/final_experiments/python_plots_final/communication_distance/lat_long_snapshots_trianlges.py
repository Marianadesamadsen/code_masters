...
RUNS = {
    f"{dt_type}dt 1": {
        "label_true": rf"$u_{{t+1\cdot {dt_type}\Delta t}}$",
        "label_ar1": rf"$\hat{{u}}^{{AR1}}_{{t+1\cdot {dt_type}\Delta t}}$",
        "label_ar2": rf"$\hat{{u}}^{{AR2}}_{{t+1\cdot {dt_type}\Delta t}}$",
        "rollout_idx": 0,
    },
    f"{dt_type}dt 2": {
...
def main():
    ...
    for run_key, cfg in RUNS.items():
        ...
        diff_ar2_ar1 = pred_ar2 - pred_ar1
    ...
    cbar2.set_label(r"$|u-\hat{u}^{AR2}|-|u-\hat{u}^{AR1}|$", fontsize=20)
