...
def plot_raw_sem_energy():
    ...
    for dt_key, cfg in RUN_DIRS.items():
        ...
        rel_energy_df, true_energy_df, pred_energy_df, energy_rollouts = (
        ...
        pred_energy_df.to_csv(
            model_save_dir / "test_energy_pred_clean_per_sample.csv",
            index=False,
        )

        energy_cols, _ = get_rollout_cols(pred_energy_df)
        ...
        if not analytical_plotted:
            ax.plot(
                ...
                color="black",
                linewidth=2,
                label="Analytical SEM energy",
            )
            analytical_plotted = True

        idx_max = np.nanargmax(y_energy_pred)
        print(f"{dt_key}: max predicted energy at rollout index {idx_max}")

    ax.set_ylabel("Energy", fontsize=20)
    ax.set_xlabel("Physical time (s)", fontsize=20)
    ...
    out_path = RESULTS_DIR / "raw_energy_sem_reference_other.png"
