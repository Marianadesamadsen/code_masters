...
def get_best_saved_sample_index(raw_dir, result_dir, metric_key="rmse"):
    ...
    # Best based on mean over all saved rollouts
    scores = metric_df.mean(axis=1)

    best_idx = int(scores.idxmin())
    best_row = metadata.iloc[best_idx]

    print("\nBest reference sample:")
    print("saved sample index:", best_idx)
    print(best_row)
    print("mean score:", scores.iloc[best_idx])

    return best_idx, best_row, float(scores.iloc[best_idx])
...
def plot_pred_target_error_for_best_train1(
    ds_geo_dir,
    run_configs,
    plot_dir,
    metric_key="rmse",
    ...
    best_idx, best_row, best_score = get_best_saved_sample_index(
        raw_dir=reference_cfg["raw_dir"],
        result_dir=reference_cfg["result_dir"],
        metric_key=metric_key,
    )

    ensemble_member = int(best_row["ensemble_member"])
    sample_idx = int(best_row["sample_idx"])
    ...
    # Use the target from train size 1 as common target for all rows
    common_target = (
        ds_reference["target"]
        .isel(sample=best_idx)
        .values[:, :, 0]
    )
    ...
    fig.suptitle(
        f"Best {metric_key.upper()} trajectory from train size {reference_train_size}\n"
    ...
    out_path = os.path.join(
        plot_dir,
        f"best_train{reference_train_size}_{metric_key}_"
...
if __name__ == "__main__":
    ...
    plot_pred_target_error_for_best_train1(
