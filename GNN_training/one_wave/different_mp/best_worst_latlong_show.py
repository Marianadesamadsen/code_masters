...
def plot_error_heatmaps_best_worst_mp_steps(
    ds_geo_dir,
    run_configs,
    plot_dir,
    metric_key="rmse",
    ...
    reference_cfg = run_configs[reference_mp_step]
    ...
    # Find same physical trajectories in all MP-step runs
    for mp_step, cfg in run_configs.items():
        ds_all = xr.open_zarr(cfg["raw_dir"])
        selected_samples[mp_step] = {}
        ...
        for group_label, ref in reference_samples.items():
            ...
            selected_samples[mp_step][group_label] = sample_idx

            if sample_idx is None:
                print(
                    f"Missing {group_label} trajectory in MP {mp_step}: "
    ...
    mp_steps = list(run_configs.keys())
    ...
    for col, mp_step in enumerate(mp_steps):
        ...
        for row, (group_label, ridx) in enumerate(row_labels):
            ...
            sample_idx = selected_samples[mp_step][group_label]
    ...
    cbar.set_label("$u_t-\hat u_t$",fontsize=20)
    cbar.ax.tick_params(labelsize=14)

    fig.suptitle(
        f"Prediction error for best/worst {metric_key} wave"
        f" selected from MP {reference_mp_step}",
        fontsize=22,
    )
    ...
    out_path = os.path.join(
        plot_dir,
        f"best_worst_{metric_key}_error_heatmaps_reference_mp"
        f"{reference_mp_step}.png",
    )
...
if __name__ == "__main__":
    ...
    run_configs = {
        ...
        3: {
            "raw_dir": (
                "GNN_training/one_wave/different_mp/"
                "test_mp3_results_new.zarr"
            ),
            "result_dir": (
                "GNN_training/one_wave/different_mp/"
                "test_mp3_results_new"
            ),
        },
    }

    plot_error_heatmaps_best_worst_mp_steps(
