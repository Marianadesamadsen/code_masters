#!/bin/sh

### General options
### –- specify queue --
#BSUB -q gpuv100

### -- set the job Name --
#BSUB -J ar1_in_training_mp10

### -- ask for number of cores (default: 1) --
#BSUB -n 10

### -- Select the resources: 1 gpu in exclusive process mode --
#BSUB -gpu "num=1:mode=exclusive_process"

### -- set walltime limit: hh:mm --  maximum 24 hours for GPU-queues right now
#BSUB -W 2:00

### request 3GB of system-memory
#BSUB -R "rusage[mem=5GB]"
#BSUB -R "span[hosts=1]"

#BSUB -u s201205@student.dtu.dk

### -- send notification at start --
#BSUB -B

### -- send notification at completion--
#BSUB -N

### -- Specify the output and error file. %J is the job-id --
### -- -o and -e mean append, -oo and -eo mean overwrite --

#BSUB -oo GNN_training/one_wave/different_AR/outputs/ar1_in_training_mp10.out
#BSUB -ee GNN_training/one_wave/different_AR/outputs/ar1_in_training_mp10.err

# -- end of LSF options --

cd /zhome/5e/a/152106/code_masters
source .venv/bin/activate 

export PYTHONPATH=/zhome/5e/a/152106/code_masters/neural-lam:$PYTHONPATH

which python
python --version
nvidia-smi 
python -m neural_lam.train_model \
    --config_path "GNN_training/one_wave/yaml_files/config_wave_28_ts_600_g4_sigmamin_15.yaml" \
    --graph "GNN_training/graphs/gsub4_msub4_nn1" \
    --loss "mse" \
    --seed 42 \
    --num_workers 0 \
    --epochs 100 \
    --processor_layers 10 \
    --logger_run_name "ar1_in_training_mp10" \
    --batch_size 32 \
    --logger-project "ar_in_training" \
    --ar_steps_train 1 \
    --precompute_in_memory True \


