...
def get_reference_element(N):
    ...
    V = vandermonde_triangle(N, r, s)
    invV = np.linalg.inv(V)

    Vr, Vs = grad_vandermonde_triangle(N, r, s)

    Dr = Vr @ invV
    Ds = Vs @ invV

    # If using the orthonormal simplex basis, this matches the Nodal-DG formulation
...
def jacobi_p(x, alpha, beta, n):
    ...
    return eval_jacobi(n, alpha, beta, x) / np.sqrt(h)
...
def rstoab(r, s):
    """Map reference triangle coordinates (r,s) to collapsed coordinates (a,b)."""
    r = np.asarray(r)
    s = np.asarray(s)
...
def simplex_2d_p(a, b, i, j):
    """Python equivalent of Simplex2DP."""
    h1 = jacobi_p(a, 0, 0, i)
...
def grad_simplex_2d_p(a, b, i, j):
    """Gradient of Simplex2DP with respect to reference coordinates r,s."""
    fa = jacobi_p(a, 0, 0, i)
    ...
    dmodedr = dfa * gb
    if i > 0:
        dmodedr *= (0.5 * (1.0 - b)) ** (i - 1)
    dmodedr *= 2.0 ** (i + 0.5)

    # derivative with respect to s
    dmodeds = dfa * gb * 0.5 * (1.0 + a)
    if i > 0:
        dmodeds *= (0.5 * (1.0 - b)) ** (i - 1)

    tmp = dgb * (0.5 * (1.0 - b)) ** i
    if i > 0:
        tmp -= 0.5 * i * gb * (0.5 * (1.0 - b)) ** (i - 1)
...
def vandermonde_triangle(N, r, s):
    """Equivalent of Vandermonde2D."""
    a, b = rstoab(r, s)
    ...
    for i in range(N + 1):
        for j in range(N + 1 - i):
            V[:, sk] = simplex_2d_p(a, b, i, j)
            sk += 1

    return V


def grad_vandermonde_triangle(N, r, s):
    """Equivalent of GradVandermonde2D."""
    a, b = rstoab(r, s)

    Np = (N + 1) * (N + 2) // 2
    Vr = np.zeros((len(r), Np))
    Vs = np.zeros((len(r), Np))

    sk = 0
    for i in range(N + 1):
        for j in range(N + 1 - i):
            Vr[:, sk], Vs[:, sk] = grad_simplex_2d_p(a, b, i, j)
            sk += 1
